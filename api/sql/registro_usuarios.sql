...
ingresos_curso AS (
    ...
    WHERE l.userid > 0
      ...
      AND l.action = 'viewed'
      AND (%(fecha_consulta_desde)s IS NULL
           OR TO_TIMESTAMP(l.timecreated)::date >= %(fecha_consulta_desde)s::date)
      AND (%(fecha_consulta_hasta)s IS NULL
           OR TO_TIMESTAMP(l.timecreated)::date <= %(fecha_consulta_hasta)s::date)
...
SELECT
    ...
    CASE
        WHEN LOWER(COALESCE(NULLIF(u.idnumber,''), u.username)) ~ '(cc|dni|ce|ppt)$'
        THEN UPPER(SUBSTRING(LOWER(COALESCE(NULLIF(u.idnumber,''), u.username)) FROM '(cc|dni|ce|ppt)$'))
        ELSE 'No definido'
    END                                                                      AS "Tipo de Documento",
    CASE
        WHEN LOWER(COALESCE(NULLIF(u.idnumber,''), u.username)) ~ '(cc|dni|ce|ppt)$'
        THEN REGEXP_REPLACE(COALESCE(NULLIF(u.idnumber,''), u.username), '(cc|dni|ce|ppt)$', '', 'i')
