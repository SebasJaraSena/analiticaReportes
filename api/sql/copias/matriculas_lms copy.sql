...
-- Presenta aprendices, instructores y demás roles enrolados y activos
-- dentro de un periodo de tiempo de un grupo/ficha.
WITH parametros AS (
    SELECT
...
SELECT
    ...
    CASE WHEN ue.status = 0 THEN 'Activo' ELSE 'Suspendido' END              AS "Estado del Usuario",
    CASE
    ...
    TO_CHAR(TO_TIMESTAMP(ue.timecreated), 'YYYY/MM/DD HH24:MI:SS')           AS "Fecha Enrolamiento",
