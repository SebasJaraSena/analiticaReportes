...
SELECT
    ...
    CASE WHEN cp.letra_modalidad IN ('V','A','P','PI') THEN 'Formación titulada' ELSE 'No definido' END AS "Nivel de formación",
    ...
    CASE WHEN COALESCE(ss.sesion_iniciada, false) THEN 'Sí' ELSE 'No' END AS "Sesión Iniciada Sí / No",
    CASE WHEN b.record = 1 OR EXISTS (
             SELECT 1
...
WHERE (%(codigo_ficha)s IS NULL OR cp.idnumber ILIKE '%%' || %(codigo_ficha)s || '%%')
  ...
  AND (%(rol_usuario)s IS NULL OR EXISTS (
       ...
       JOIN public.mdl_role_assignments ra ON ra.userid = blr.userid AND ra.contextid = ctx.id
  ...
  AND (%(identificacion)s IS NULL OR EXISTS (
       ...
       WHERE bl2.bigbluebuttonbnid = b.id
         AND u2.username ILIKE '%%' || %(identificacion)s || '%%'
  ))
  ...
  AND (%(fecha_desde)s IS NULL OR (b.openingtime > 0 AND TO_TIMESTAMP(b.openingtime)::date >= %(fecha_desde)s::date))
  AND (%(fecha_hasta)s IS NULL OR (b.openingtime > 0 AND TO_TIMESTAMP(b.openingtime)::date <= %(fecha_hasta)s::date))
  AND (%(fecha_inicio_grupo_desde)s IS NULL OR TO_TIMESTAMP(cp.startdate)::date >= %(fecha_inicio_grupo_desde)s::date)
  AND (%(fecha_inicio_grupo_hasta)s IS NULL OR TO_TIMESTAMP(cp.startdate)::date <= %(fecha_inicio_grupo_hasta)s::date)
  AND (%(fecha_creacion_grupo_desde)s IS NULL OR TO_TIMESTAMP(cp.curso_timecreated)::date >= %(fecha_creacion_grupo_desde)s::date)
  AND (%(fecha_creacion_grupo_hasta)s IS NULL OR TO_TIMESTAMP(cp.curso_timecreated)::date <= %(fecha_creacion_grupo_hasta)s::date)
ORDER BY cp.idnumber, b.openingtime DESC, b.name
